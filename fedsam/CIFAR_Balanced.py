...
from Neural_Networks import train_models, cnn_2layer_fc_model, cnn_3layer_fc_model, Resnet20
...
if __name__ == "__main__":
    ...
    # Qui viene caricato il file di conf, dove sono presenti tutti i modelli delle reti delle 10 parti da allenare
    # con i relativi dati, le classi private e pubbliche e così via.
    conf_file =  parseArg()
    ...
    #Qua va a caricare cifar10 come public dataset, X e Y sono rispettivamente il dato e la label associata 
    #Invece usa cifar100 come private prendendo solo le label associate a cifar10
    ...
    for index, cls_ in enumerate(private_classes):        
        y_train_CIFAR100[y_train_CIFAR100 == cls_] = index + len(public_classes) #10,11,12,13..
        y_test_CIFAR100[y_test_CIFAR100 == cls_] = index + len(public_classes)
    del index, cls_
    
    print(pd.Series(y_train_CIFAR100).value_counts())
    mod_private_classes = np.arange(len(private_classes)) + len(public_classes) # array([10, 11, 12, 13, 14, 15])
    
    print("="*60)
    #generate private data => immagini associate alle 10 labels scelte
    ...
    #qui inizia l'algoritmo di fedMD.
    #all'inizio tutti i partecipanti pretrainano il loro modello sul dataset pubblic cifar10
    ...
    fedmd = FedMD(parties, 
                  public_dataset = public_dataset, #X-Y train cifar10
                  private_data = private_data, #X-Y train cifar 100 per ogni parties
                  total_private_data = total_private_data, #BOH
                  private_test_data = private_test_data, #X-Y test cifar 100 
                  N_rounds = N_rounds,
